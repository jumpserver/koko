...
func AssetProfileProbeCommand() string {
	return `if [ -r /etc/os-release ]; then . /etc/os-release; fi; ` +
		`printf 'os_name=%s\nos_id=%s\nversion_id=%s\nkernel=%s\narchitecture=%s\n' "${PRETTY_NAME:-${NAME:-unknown}}" "${ID:-unknown}" "${VERSION_ID:-unknown}" "$(uname -sr 2>/dev/null || uname -a 2>/dev/null || printf unknown)" "$(uname -m 2>/dev/null || printf unknown)"; ` +
...
func ParseAssetProfile(output string) AssetProfile {
	profile := AssetProfile{
		...
		Kernel: "unknown", Architecture: "unknown",
	}
