...
func Register(name string, factory Factory) error {
	...
	if factory == nil {
		return fmt.Errorf("agent runtime provider %q factory is required", name)
	}
	registry.Lock()
	defer registry.Unlock()
	if _, exists := registry.factories[name]; exists {
		return fmt.Errorf("agent runtime provider %q is already registered", name)
	}
...
func New(config Config) (Provider, error) {
	if strings.TrimSpace(config.APIKey) == "" || strings.TrimSpace(config.Model) == "" {
		return nil, fmt.Errorf("agent runtime model is not configured")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("initialize agent runtime provider %q: %w", config.Name, err)
	}
	if result == nil {
		return nil, fmt.Errorf("agent runtime provider %q factory returned nil", config.Name)
	}
	return result, nil
}
