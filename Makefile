...
linux-s390x:koko-ui
	GOARCH=s390x GOOS=linux $(KOKOBUILD) -o $(BUILDDIR)/$(NAME)-$@ $(KOKOSRCFILE)
	GOARCH=s390x GOOS=linux $(K8SCMDBUILD) -o $(BUILDDIR)/kubectl-$@ $(KUBECTLFILE)
	GOARCH=s390x GOOS=linux $(K8SCMDBUILD) -o $(BUILDDIR)/helm-$@ $(HELMFILE)
	mkdir -p $(BUILDDIR)/$(NAME)-$(VERSION)-$@/locale/
...
linux-ppc64le:koko-ui
	GOARCH=ppc64le GOOS=linux $(KOKOBUILD) -o $(BUILDDIR)/$(NAME)-$@ $(KOKOSRCFILE)
	GOARCH=ppc64le GOOS=linux $(K8SCMDBUILD) -o $(BUILDDIR)/kubectl-$@ $(KUBECTLFILE)
	GOARCH=ppc64le GOOS=linux $(K8SCMDBUILD) -o $(BUILDDIR)/helm-$@ $(HELMFILE)
	mkdir -p $(BUILDDIR)/$(NAME)-$(VERSION)-$@/locale/
